...
write.config.SIPNET <- function(defaults, trait.values, settings, run.id, inputs = NULL, IC = NULL,
  ...
  if (!is.null(IC)) {
    ...
    if (all(plant_wood_vars %in% ic.names)) {
      IC$AbvGrndWood <- IC$AbvGrndWood*1000   ## TODO: THIS IS A temporary hack; NEED TO REMOVED LATER
    ...
    ## neeInit gC/m2
    if ("NEE" %in% ic.names) {
      param[which(param[, 1] == "neeInit"), 2] <- PEcAn.utils::ud_convert(IC$NEE, 'kg m-2', 'g m-2')
    }
    ## laiInit m2/m2
    if ("LAI" %in% ic.names) {
