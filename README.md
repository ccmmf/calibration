...
Every path in an example config is built from that root

For the joint calibration example, the inputs and the calibration result are
published so the run can be reproduced or audited without cluster access:
